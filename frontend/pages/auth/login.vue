    ...
    <div class="login h-full w-full pt-12">
        <div class="w-1/2 card m-auto">
            ...
            <div class="card-content">
                ...
                <div class="login-form flex flex-col justify-start items-start space-y-5 py-6">
                    ...
                    <form action="#" @submit="login" class="w-full flex flex-col space-y-5 items-start jus">
                        <div class="input-control">
                            <label>Email:</label>
                            <input v-bind="email" type="email" placeholder="Email" class="input" :error="(typeof errors.email === 'string')"/>
                            <span class="error-text-alt">{{ errors.email }}</span>
                        </div>

                        <div class="input-control">
                            <label>Password:</label>
                            <input v-bind="password" type="password" placeholder="Password" class="input" :error="typeof errors.password === 'string'"/>
                            <span class="error-text-alt">{{ errors.password }}</span>
                        </div>
                    </form>


                    <span class="w-full text-center font-primary font-medium">No account? <NuxtLink to='/auth/register' class="ghost">Click to create new account</NuxtLink></span>
                </div>

                <div class="card-footer flex flex-row justify-end">
                    <button @click.prevent="login()" :disabled="!meta.valid || isSubmitting">Login</button>
                </div>
...
const { handleSubmit, defineInputBinds, meta, errors, isSubmitting } = useForm<LoginDto>({
...
const login = handleSubmit(async (data) => {
    const response = await storeLogin(data.email, data.password);
