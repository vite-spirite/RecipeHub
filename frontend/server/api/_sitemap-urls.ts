...
export default defineEventHandler(async () => {
    ...
    const apiUrl = runtimeConfig.public.generateApi;

    const cresponse = await fetch(apiUrl+"/category");
    const categories = await cresponse.json();
    
    const recipes: CompactRecipeDto[] = [];

    for(const category of categories) {
        const rresponse = await fetch(`${apiUrl}/recipe/category/${category.id}/1`);
        const _recipes = await rresponse.json();
        
        //recipes.push(..._recipes.data.value.data);
        for(let page = 1; page <= _recipes.lastPage; page++) {
            const sresponse = await fetch(`${apiUrl}/recipe/category/${category.id}/${page}`);
    ...
    const uresponse = await fetch(apiUrl+"/users");
